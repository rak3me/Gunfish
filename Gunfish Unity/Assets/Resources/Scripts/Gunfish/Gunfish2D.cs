...
//			flopAudioSource.volume = Mathf.Min (-rb.velocity.y / 100, 1f);
	...
	void Update () {
        if (!isLocalPlayer) return; //Only give authority to the appropriate player
        ...
        if (serverInput != 0) {
            NetworkManager.singleton.client.Send(MessageTypes.INPUTMSG, new InputMsg(serverInput, false, gameObject));
        }
	}
